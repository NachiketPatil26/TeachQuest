...
import { Edit, Trash2, Plus, X, Search, Download } from 'lucide-react';
...
export default function TeacherInfoPage() {
  ...
  const handleExportToExcel = async () => {
    try {
      ...
      // Prepare data with all information in a single sheet
      const excelData = teachers.map(teacher => {
        ...
        const allocationDetails = allocations.map(allocation => {
          const assignedBlock = allocation.blocks?.find(block => 
            block.invigilator?._id === teacher._id
          );
          ...
          return {
            ...
            'Exam Name': allocation.examName,
            'Subject': allocation.subject,
            'Date': new Date(allocation.date).toLocaleDateString(),
            'Time': `${allocation.startTime} - ${allocation.endTime}`,
            'Block': assignedBlock ? `Block ${assignedBlock.number}` : 'Not assigned',
            'Location': assignedBlock?.location || 'N/A'
          };
        });

        // If teacher has no allocations, add one row with basic info
      ...
      // Iterate through rows to find repeated values
      for (let row = 1; row <= lastRow; row++) {
        const currentName = ws[XLSX.utils.encode_cell({ r: row, c: 0 })]?.v;
        const nextName = row < lastRow ? ws[XLSX.utils.encode_cell({ r: row + 1, c: 0 })]?.v : null;

        // If we're at the last row or the next row has a different name
        if (row === lastRow || currentName !== nextName) {
          // Merge cells for this teacher's information
          for (let col = 0; col < 4; col++) { // Merge first 4 columns (Teacher Name, Email, Department, Subjects)
            ws['!merges'].push({
      ...
      XLSX.utils.book_append_sheet(wb, ws, 'Teacher Allocations');

      // Generate Excel file
      XLSX.writeFile(wb, `teacher-allocations-${branch || 'all'}.xlsx`);

    } catch (err) {
      const errorMessage = (err as Error & { response?: { data?: { message?: string } } }).response?.data?.message || 'Failed to export data';
      setError(errorMessage);
    ...
    <div className="min-h-screen bg-gray-50 py-8 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="md:flex md:items-center md:justify-between mb-8">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Teacher Information</h1>
