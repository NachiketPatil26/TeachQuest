...
const isTeacherAvailable = (teacher: Teacher, examDate: string): boolean => {
  ...
  const weekday = new Date(examDate).toLocaleDateString("en-US", {
    weekday: "long",
  });
...
export default function TeacherAllocation() {
  ...
  const confirmAllocation = async () => {
    try {
      ...
      console.log("Allocating teachers:", {
        examId: selectedSlot,
        ...
        try {
          await fetch("/api/notifications", {
            ...
            body: JSON.stringify({
              ...
              } exam on ${new Date(selectedExam.date).toLocaleDateString(
                "en-US",
                {
                  weekday: "long",
                  year: "numeric",
              ...
              )} from ${selectedExam.startTime} to ${
                selectedExam.endTime
              } in Block ${selectedExam.block}.`,
              type: "info",
              relatedTo: {
                model: "Exam",
                id: selectedSlot,
      ...
      const errorMessage =
        error instanceof Error ? error.message : "Failed to allocate teachers";
  ...
  const handleTeacherSelection = (teacherId: string) => {
    ...
    if (isSelected) {
      ...
      if (!isAlreadyAllocated) {
        ...
        const isAllocatedToOtherSlot = examSlots.some(
          (otherSlot) =>
        ...
        if (!isAllocatedToOtherSlot) {
          ...
          setError(
            `Teacher ${teacher.name} is already allocated to another exam slot`
          );
        }
      } else {
        setError(
          `Teacher ${teacher.name} is already allocated to this exam slot`
        );
      }
    }
  };
    ...
    <div className="min-h-screen bg-gray-50 pb-8">
      ...
      <div className="pt-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
          ...
          <div className="fixed inset-0 bg-gray-500 bg-opacity-75 flex items-center justify-center z-50">
            <div className="bg-white p-6 rounded-lg shadow-xl max-w-md w-full">
              <h3 className="text-lg font-medium mb-4">
                Confirm Teacher Allocation
              </h3>
              <p className="text-gray-600 mb-4">
                Are you sure you want to allocate {selectedTeachers.length}{" "}
                teacher(s) to this exam slot?
              </p>
        ...
        <div className="pt-20 md:flex md:items-center md:justify-between mb-8">
          <div>
            <h2 className="text-2xl -mt-2 font-bold text-gray-900">{branch}</h2>
          </div>
        </div>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <button
                          onClick={(e) => {
                            ...
                            const selectedExamSlot = examSlots.find(
                              (s) => s._id === examSlot._id
                            );
                            if (!selectedExamSlot) {
                            ...
                            if (
                              selectedExamSlot.allocatedTeachers &&
                              selectedExamSlot.allocatedTeachers.length > 0
                            ) {
                              setSelectedTeachers(
                                selectedExamSlot.allocatedTeachers
                              );
                          ...
                          className="text-[#2A4F8F] hover:text-[#1E365E]"
                        >
                          {examSlot.allocatedTeachers &&
                          ...
                          >
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                              {teachers.map((teacher) => {
                                const isAvailable = isTeacherAvailable(
                                  teacher,
                                  new Date(examSlot.date)
                                    .toISOString()
                                    .split("T")[0]
                                );
                                const hasSubjectConflict =
                                  isTeacherSubjectConflict();
                                const isAllocated = selectedTeachers.includes(
                                  teacher._id
                                );
                                  ...
                                  >
                                    <div className="flex items-start justify-between">
                                      <div>
                                        ...
                                        {isAllocated && (
                                          <p className="text-xs text-[#9FC0AE] mt-1">
                                            Currently Allocated
                                          </p>
                                        )}
                                        <div className="mt-2">
                                      ...
                                      <button
                                        onClick={() =>
                                          handleTeacherSelection(teacher._id)
                                        }
                                        disabled={
                                          !isAvailable || hasSubjectConflict
                                        }
                                        className={`p-2 rounded-full transition-all duration-200 ${
                                          selectedTeachers.includes(teacher._id)
                                            ? "bg-[#2A4F8F] text-white scale-110"
                                            : "bg-gray-100"
                                        } ${
                                          !isAvailable || hasSubjectConflict
                                            ? "opacity-50 cursor-not-allowed"
                                            : "hover:bg-[#2A4F8F] hover:scale-105"
                                        }`}
                                      >
                            ...
                            <div className="mt-6 flex justify-end">
                              ...
                              <button
                                ...
                                className="px-4 py-2 bg-[#2A4F8F] text-white rounded-md hover:bg-[#1E365E] disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 hover:scale-105"
                              >
